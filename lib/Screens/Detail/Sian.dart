...
class Sian extends StatefulWidget {
  Sian({Key key, this.title}) : super(key: key);

  final String title;

  @override
  _SianState createState() => _SianState();
}

class _SianState extends State<Sian> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      bottomNavigationBar: BottomAppBar(
        color: Color(0xFF606837),
        child: Container(
            height: MediaQuery.of(context).size.height * 0.05,
            child: Center(
              child: Text(
                "Create Your SIÁN FKP 37",
                style: TextStyle(
                  fontSize: 20,
                  color: Colors.white,
  ...
  void updateImageList(BuildContext context) async {
    for (int i = 1; i <= 50; i++) {
      imageList.add(AssetImage('lib/assets/images/SianImages/$i.png'));
      //* To precache images so that when required they are loaded faster.
      await precacheImage(
          AssetImage('lib/assets/images/SianImages/$i.png'), context);
    }
    setState(() {
      imagePrecached = true;
    });
